...
hyperGraph = sortWith snd . DM.elems . foldl f DM.empty . zip ([0..] :: [Int]) where
  f hgmap (i, k)= DM.alter (Just . maybe (k, [i]) (appendToSnd i)) k hgmap where
