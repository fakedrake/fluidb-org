...
squashDatesE :: forall e' e s .
               SymEmbedding (ExpTypeSym' e) s e'
             -> Expr e' -> Maybe (Expr e')
squashDatesE SymEmbedding{..} = qmapM' $ \case
  e@(E2 o el er) -> case (nonSymName el,nonSymName er) of
    (Just (EDate dl), Just (EInterval dr)) ->
      E0 . embedLit . EDate <$> (dl `dateAppend` dr) False
    (Just (EInterval dl), Just (EInterval dr)) ->
      E0 . embedLit . EDate <$> (dl `dateAppend` dr) True
      ...
      dateAppend dl ((if o == ESub then negDate else id) -> dr) isInterval =
        ...
        where
          ...
          goAll = do
            ...
            let time = zeroDay{seconds=seconds', minute=minute', hour=hour'}
            dayCarry <- get
            put 0
            ...
            if isInterval
              then return time{year=year', month=month', day=lday+rday}
...
squashDates :: SymEmbedding (ExpTypeSym' e) s e'
            -> Query e' s -> Maybe (Query e' s)
