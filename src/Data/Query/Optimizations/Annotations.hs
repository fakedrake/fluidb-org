...
annotateTypes :: forall e' s s' e uid err m .
              ...
              -> Query e' s
              -> m (Query (e',CppType) s)
annotateTypes eqE colType litType = recur where
  ...
  aggT = expT . fmap (((),) . aggT0) . fmap2 expT where
  ...
  sT e = fromJust' $ litType' e <|> listToMaybe (mapMaybe go qSchema)
    where
      ...
      fromJust' = maybe
        (throwAStr $ "Failed to deduce symbol type."
          ++ ashow (fst e,redactSch qSchema))
        return
      redactSch = fmap
        $ bimap (const ())
        $ bimap (bimap (fmap (bimap (first fst)
                              $ fmap $ first fst))
                 $ const ())
      ...
      luE = listToMaybe . filter (eqE e . fst)
