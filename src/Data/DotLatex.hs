...
sanitized = putStyle . mergeColors where
  ...
  mergeColors = first (mconcat . catMaybes . fmap justCol)
                . partition (\case {DBackground _ -> True; _ -> False})
    where
...
instance ToDot DotAst where
  toDot' = \case
    ...
    DProp x -> toDot' x
    DGraph s p -> do
      isDir <- (== Directed) . structureDirected <$> ask
