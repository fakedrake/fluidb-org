...
idempotentClusterInsert constraints m = do
  ...
  where
    toSpecificClustLocal :: AnyCluster e s t n
                         -> Maybe
                         (c NodeRef (ComposedType c (PlanSym e s) NodeRef) t n)
...
cachedMkRef fromCache toCache cnf = fromCache <$> get >>= \case
  Nothing -> do {x <- lift mkRef; modify $ toCache x; return x}
  Just ref -> lift (linkNRefCnfs ref [cnf]) >> return ref
  where
...
runCnfCache :: Monad m =>
            ...
            -> CGraphBuilderT e s t n m [a]
runCnfCache v =
  runStateT (runListT v) . cnfBuildCache . clustBuildCache <$> get >>= \case
