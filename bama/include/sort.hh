...
class MergeSort {
 ...
 public:
  MergeSort(const OutputPrim& out,
            ...
            const std::string& fn, size_t nb = 5)
      : in_filename(fn),
        temp_prefix(out.value),
        out_filename(out),
        number_of_runs(0),
 ...
 private:
  ...
  void merge_runs() {
    ...
    do {
      ...
      fname = processed_so_far != number_of_runs ? generate_run()
                                                 : out_filename.value;
      output.open(fname);
  ...
  std::pair<unsigned int, unsigned int> pick_runs(
      size_t processed_so_far) const {
...
template <typename ToKeyFn, typename OutputPrim>
class InlineSort {
 private:
  typedef typename ToKeyFn::Domain0 Record;
  typedef typename ToKeyFn::Codomain KeyRecord;
 public:
  InlineSort(const OutputPrim& out, const std::string& fn)
      : in_filename(fn), out_filename(out) {}
  ~InlineSort() {}
  ...
  void run() {
    copyFile(in_filename.c_str(), out_filename.value.c_str());
    sortFile<ToKeyFn>(out_filename.value);
  }
...
template <typename ToKeyFn,
          typename OutputPrim,  // Maybe
          typename OutputSec>
auto mkSort(const OutputPrim& out, const OutputSec& outsec,
            const std::string& fn) {
  return InlineSort<ToKeyFn, OutputPrim>(out, fn);
}
#endif /* SORT_H */
