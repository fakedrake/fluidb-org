...
instance BndRParams (MulTag p a) where
  ...
  bndLt = error "not implemented"
  exceedsCap = error "not implemented"


instance (ExtParams (MulTag p a) p,AShow a,Zero (Mul a),AShow (ExtCoEpoch p),Eq a,Num a,Integral a)
  ...
  zprocEvolution =
    ...
    { evolutionControl = mulSolution
     ,evolutionStrategy = error "not implemented"
  ...
  zLocalizeConf coepoch conf z =
    ...
    where
      newCap = fromZ (prRes $ zRes z) $ case confCap conf of
        ...
        x -> x
      fromZ r x = case r of
        Just (Left _)          -> CapVal zero
        Just (Right (Mul 0 _)) -> x
        Just (Right _is_zero)  -> CapVal zero
        _                      -> x
